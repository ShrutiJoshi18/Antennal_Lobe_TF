...
g_ach = [0.03]*p_n+[0.05]*l_n         # Ach Conductance
...
g_fgaba = [0.2]*p_n+[0.6]*l_n        # fGABA Conductance
E_fgaba = [-70.0]*n_n                # fGABA Potential


# Facilitation Parameters #

p_0 = [1.0]*n_syn_ach + [0.1]*n_syn_fgaba_pn + [0.1]*n_syn_fgaba_ln
p_tau = [15000]*n_syn_ach + [10000]*n_syn_fgaba_pn + [13000]*n_syn_fgaba_ln
del_p = [0]*n_syn_ach + [8]*n_syn_fgaba_pn + [0]*n_syn_fgaba_ln
...
if sys.argv[1] == '0':
    state_vector =  [-30]* n_n + [0.0]* n_n + [0.0]* (4*p_n) + [0.0]* (3*l_n) + [2.4*(10**(-4))]*l_n + [0]*(n_syn_ach) + [0]*(n_syn_fgaba) + [1.0]*(n_syn_ach) + [0.1]*(n_syn_fgaba) +  [-(sim_time+1)]*n_n
    state_vector = np.array(state_vector)
    state_vector = state_vector + 0.02*state_vector*np.random.normal(size=state_vector.shape)
...
t_ = time.time()
indices = [i for i in range(n_n)] + [i for i in range(6*n_n + n_syn_ach + n_syn_fgaba,6*n_n + 2*n_syn_ach + 2*n_syn_fgaba)]
...
for n,i in enumerate(t_batch):
    ...
    np.save("batch"+str(int(sys.argv[1])+1)+"_part_"+str(n+1),state[::10,indices])
